...
func TestHelmChartRenderServerModule(t *testing.T) {
	...
	// Parse rendered YAML to verify server resources
...
func TestHelmChartRenderWithIngress(t *testing.T) {
	...
	// Note: Ingress might not always be created depending on chart logic
	if ingressFound {
		t.Log("Ingress/Route found in rendered output")
	}
}
...
func TestHelmChartLint(t *testing.T) {
	...
	// Helm lint may not be available in test environment
	if err != nil {
		t.Skipf("helm lint not available: %v", err)
	}

	assert.NoError(t, err, "helm lint should pass")
	assert.Contains(t, string(output), "1 chart(s) linted", "chart should be linted successfully")
}
